module GroupDocs
  class Signature::Template < Api::Entity
    ...
    #
    # Returns documents array of template.
    #
    # @param [Hash] access Access credentials
    # @option access [String] :client_id
    # @option access [String] :private_key
    ...
    def documents!(access = {})
      json = Api::Request.new do |request|
        request[:access] = access
        request[:method] = :GET
        request[:path] = "/signature/{{client_id}}/templates/#{id}/documents"
      end.execute!
    ...
    def add_document!(document, options = {}, access = {})
      ...
      api = Api::Request.new do |request|
        request[:access] = access
        request[:method] = :POST
        request[:path] = "/signature/{{client_id}}/templates/#{id}/document/#{document.file.guid}"
      end
      api.add_params(options)
      api.execute!
    end
    ...
    #
    # Removes document from template.
    #
    # @example
    #   template = GroupDocs::Signature::Template.get!("g94h5g84hj9g4gf23i40j")
    ...
    def remove_document!(document, access = {})
      ...
      Api::Request.new do |request|
        request[:access] = access
        request[:method] = :DELETE
        request[:path] = "/signature/{{client_id}}/templates/#{id}/documents/#{document.file.guid}"
      end.execute!
    end
